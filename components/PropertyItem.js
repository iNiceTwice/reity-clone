...
const PropertyItem = ({ data }) => {
    ...
    return ( 
        <>
            <Link href={`/propiedades/${data._id}`}>
                <div className="rounded-lg overflow-hidden">
                    <div className="relative w-full h-[26rem] lg:h-[20vw]">
                        ...
                        <div className="absolute w-full h-full hover:bg-white/40 z-20 flex">
                            <div className="self-end p-4 text-sm bg-white/90 w-fit rounded-full text-main m-4">
                                ${ new Intl.NumberFormat('de-DE').format(data.tokens.price) } inversión mínima
                            </div>
                        </div>
                        ...
                        <div className="flex justify-between">
                            ...
                            <div>
                                ...
                                <div className="flex font-thin gap-2 items-center">
                                    {
                                        data.boost > 0 ?
                                        <p className="text-sm line-through text-slate-800/70">{ data.capRate }%</p>
                                        :
                                        null
                                    }
                                    <p className="text-orange-500/80">{ data.capRate + data.boost }% Cap Rate</p>
                                </div>
                            </div>
                        </div>
